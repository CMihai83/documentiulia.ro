...
test.describe('ANAF Critical Flows - VAT Compliance', () => {
  ...
  test.describe('Reverse Charge API (Art. 331)', () => {
    ...
    test('POST /vat/reverse-charge should check applicability', async ({ request }) => {
      ...
      const data = await response.json();
      expect(data.applies).toBeDefined();
      expect(typeof data.applies).toBe('boolean');
    });
  ...
  test.describe('VAT Calculation API', () => {
    test('POST /vat/calculate should calculate VAT correctly', async ({ request }) => {
...
test.describe('ANAF Critical Flows - SAF-T D406', () => {
  test.describe('SAF-T D406 Deadlines API (Ordin 1783/2021)', () => {
    test('GET /saft-d406/deadlines should return deadline info', async ({ request }) => {
    ...
    test('SAF-T deadlines response time should be under 500ms (cached)', async ({ request }) => {
      const start = Date.now();
      await request.get(`${API_BASE}/saft-d406/deadlines`);
      const duration = Date.now() - start;
      expect(duration).toBeLessThan(500);
    });

    test('Grace period should be correctly indicated', async ({ request }) => {
      const response = await request.get(`${API_BASE}/saft-d406/deadlines`);
      expect(response.status()).toBe(200);
...
test.describe('ANAF Critical Flows - Error Handling', () => {
  test('Invalid VAT calculation should return validation error', async ({ request }) => {
    const response = await request.post(`${API_BASE}/vat/calculate`, {
      data: {
  ...
  test('Non-existent deadline should return 404', async ({ request }) => {
