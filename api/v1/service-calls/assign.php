...
try {
    ...
    if (!$input || empty($input['call_id']) || empty($input['technician_id'])) {
        throw new Exception('call_id and technician_id required');
    }

    $call = $serviceCallService->assignTechnician(
        $companyId,
        $input['call_id'],
        $input['technician_id'],
