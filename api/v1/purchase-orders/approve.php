<?php
/**
 * Approve Purchase Order API Endpoint
 *
 * Approves a purchase order and updates status
 *
 * @endpoint /api/v1/purchase-orders/approve.php
 * @method POST
 */
...
try {
    $po = $poService->approvePurchaseOrder($companyId, $input['id'], $userData['user_id']);
    sendSuccess(['purchase_order' => $po], 'Purchase order approved successfully');
} catch (Exception $e) {
    error_log('Approve PO error: ' . $e->getMessage());
    sendError('Failed to approve purchase order', 500);
}
