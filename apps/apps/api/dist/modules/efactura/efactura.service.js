  ...
  ${lines}
    ...
    async getStatusSummary(companyId) {
        ...
        statuses.forEach((s) => {
            ...
            if (status === client_1.EfacturaStatus.PENDING)
                summary.pending = s._count;
            else if (status === client_1.EfacturaStatus.PROCESSING)
                summary.processing = s._count;
            else if (status === client_1.EfacturaStatus.VALIDATED)
                summary.accepted = s._count;
            else if (status === client_1.EfacturaStatus.REJECTED)
                summary.rejected = s._count;
            else if (!status)
                summary.notSent = s._count;
        });
        return summary;
    }
