...
export class SaftService {
  ...
  async generateSaftXml(companyId: string, dto: SaftExportDto, userId: string): Promise<string> {
    ...
    // Fetch all required data based on export type
  ...
  async validateSaftData(companyId: string, dto: SaftExportDto, userId: string): Promise<SaftValidationResultDto> {
    await this.checkCompanyAccess(companyId, userId);
  ...
  private async fetchInvoices(companyId: string, startDate: Date, endDate: Date) {
  ...
  private async fetchClients(companyId: string) {
    return this.prisma.client.findMany({
      where: { companyId },
    });
  }

  private async fetchProducts(companyId: string) {
  ...
  private buildInvoiceEntry(invoice: any) {
    return {
      ...
      'n1:Line': invoice.items.map((item: any, index: number) =>
        this.buildInvoiceLine(item, index + 1),
      ),
    };
  }

  private buildInvoiceLine(item: any, lineNumber: number) {
  ...
  async getExportHistory(companyId: string, userId: string) {
    await this.checkCompanyAccess(companyId, userId);

    // This would return historical exports if we were storing them
